...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function applyFilters() {
        ...
        let baseOrders = viewableOrders;
        if (userType === 'ad mind' || userType === 'Nhà máy tôn') {
             // Admin và NMT có thể lọc trong danh sách họ phụ trách
             baseOrders = viewableOrders;
        } else {
            // Khách hàng/Cửa hàng đã được lọc sẵn, không cần lọc lại tên
        }

        let filteredByDateAndStatus = baseOrders.filter(order => {
            const orderDate = new Date(order['thời gian lên đơn']);
        ...
        if (customer && (userType === 'ad mind' || userType === 'Nhà máy tôn')) {
    ...
    function showOrderDetails(orderId) {
        ...
        document.getElementById('modalOrderInfo').innerHTML = `
            ...
            <p><strong>Ngày tạo:</strong> ${order['thời gian lên đơn'] || ''}</p>
            <p><strong>Trạng thái:</strong> ${order['trạng thái'] || ''}</p>
            <p><strong>Tổng tiền:</strong> ${formatVND(order['tổng phụ'])}</p>
            <p><strong>Còn lại:</strong> ${formatVND(order['số tiền còn lại trong đơn'])}</p>
        `;
    ...
    async function initializeApp() {
        try {
            ...
            } else if (['Nhà máy tôn', 'Cửa Hàng'].includes(userType) || !['ad mind'].includes(userType)) {
            ...
            if (ordersTbody) ordersTbody.innerHTML = `<tr><td colspan="8">Lỗi tải dữ liệu.</td></tr>`;
        }
    }
